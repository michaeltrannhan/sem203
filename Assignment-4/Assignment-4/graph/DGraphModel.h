...
public:
    ...
    void disconnect(T from, T to){
        ...
        typename AbstractGraph<T>::Edge *edge =nodeF->getNode(nodeT);
    ...
    void remove(T vertex){
        ...
        while(nodeIt != this->nodeList.end()){
            ...
            typename AbstractGraph<T>::Edge *egde = nodeB->getEdge(nodeA);
            if(egde != 0) nodeB->removeTo(nodeA);
            egde = nodeA->getEdge(nodeB);
            if(egde != 0) nodeA->removeTo(nodeB);
            nodeIt++;
        }
